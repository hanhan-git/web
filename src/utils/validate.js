...
/**
 * 字段验证
 * 首字符为字母，其它只允许为字母、数字或下划线，并且长度不超过40字符"
 * @param {*} value
 * @param {*} callback
 */
export function testField(rule, value, callback) {
  const res = /^[A-Za-z]{1}([a-zA-Z0-9_]{1,39})$/
  if (!res.test(value) && value.length < 41) {
    callback(new Error('首字符为字母，其它只允许为字母、数字或下划线!'))
  } else if ((!res.test(value) || res.test(value)) && value.length > 41) {
    callback(new Error('长度不超过40字符'))
  } else {
    callback()
  }
}
