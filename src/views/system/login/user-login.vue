...
<template>
  <el-form ref="loginForm" :rules="loginRules" :model="loginForm" class="login-form" status-icon label-width="0" @submit.native.prevent>
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          const formClone = JSON.parse(JSON.stringify(this.loginForm));
          formClone.username = encrypt(this.loginForm.username)
          formClone.password = encrypt(this.loginForm.password)
