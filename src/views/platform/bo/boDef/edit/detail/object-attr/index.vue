...
export default {
  ...
  methods: {
    ...
    handleEdit(id = '', readonly = false) {
      ...
      if (index > -1 && this.listData[index].code === 'id') {
        this.$message({
          message: '主键不可编辑！',
          type: 'warning'
        })
      } else if (index > -1 && this.listData[index].code === 'tenantId') {
        this.$message({
          message: '租户ID不可编辑！',
          type: 'warning'
        })
      } else if (index > -1 && this.listData[index].code === 'ip') {
        this.$message({
          message: 'IP地址不可编辑！',
          type: 'warning'
        })
      } else if (index > -1 && this.listData[index].code === 'createBy') {
        this.$message({
          message: '创建人不可编辑！',
          type: 'warning'
        })
      } else if (index > -1 && this.listData[index].code === 'createTime') {
        this.$message({
          message: '创建时间不可编辑！',
          type: 'warning'
        })
      } else if (index > -1 && this.listData[index].code === 'updateBy') {
        this.$message({
          message: '更新人不可编辑！',
          type: 'warning'
        })
      } else if (index > -1 && this.listData[index].code === 'updateTime') {
        this.$message({
          message: '更新时间不可编辑！',
          type: 'warning'
        })
    ...
    callback(value, buttonKey) {
      const codeIndex = this.listData.findIndex(i => i.code.toLowerCase() === value.code.toLowerCase())
      const fieldNameIndex = this.listData.findIndex(i => i.fieldName.toLowerCase() === value.fieldName.toLowerCase())
    ...
    handleRemove(ids) {
      ...
      arr.forEach(a => {
        this.listData.forEach((i, index) => {
          if (a === i.id && i.code !== 'id' && i.code !== 'tenantId' && i.code !== 'ip' && i.code !== 'createBy' && i.code !== 'createTime' && i.code !== 'updateBy' && i.code !== 'updateTime') {
            this.listData.splice(index, 1)
          } else if (a === i.id && i.code === 'id') {
            this.$message({
              message: '主键不可删除！',
              type: 'warning'
            })
          } else if (a === i.id && i.code === 'tenantId') {
            this.$message({
              message: '租户ID不可删除！',
              type: 'warning'
            })
          } else if (a === i.id && i.code === 'ip') {
            this.$message({
              message: 'IP地址不可删除！',
              type: 'warning'
            })
          } else if (a === i.id && i.code === 'createBy') {
            this.$message({
              message: '创建人不可删除！',
              type: 'warning'
            })
          ...
          } else if (a === i.id && i.code === 'updateBy') {
            this.$message({
              message: '更新人不可删除！',
              type: 'warning'
            })
          } else if (a === i.id && i.code === 'updateTime') {
            this.$message({
              message: '更新时间不可删除！',
              type: 'warning'
            })
