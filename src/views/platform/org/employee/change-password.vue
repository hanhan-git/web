...
import {  userSecurityTextObj, regx } from '../employee/constants'
import { mapActions } from 'vuex'
import ActionUtils from '@/utils/action'
import { encrypt } from '@/api/oauth2/user'
  ...
  methods: {
    ...
    saveData() {
      ...
      if (this.$store.getters.regOpen) {
        ...
        const passwordClone = JSON.parse(JSON.stringify(this.password));
        passwordClone.primitivePassword = encrypt(this.password.primitivePassword)
        passwordClone.newPassword = encrypt(this.password.newPassword)
        passwordClone.repeatPassword = encrypt(this.password.repeatPassword)
        
        changePassword(passwordClone).then(response => {
          ActionUtils.saveSuccessMessage(response.message, (rtn) => {
            if (rtn) {
              this.closeDialog()
            }
    ...
    handleGetDefaultUserSecurity() {
      ...
      getDefaultUserSecurity().then(response => {
        const userSecurityTextObj = that.userSecurityTextObj
        const complexityArr = response.data.complexity.split(',').sort()
        const arr = []
        complexityArr.forEach(i => {
          arr.push(userSecurityTextObj[parseInt(i)])
        })
        that.userSecurity.complexity = complexityArr
        that.userSecurity.complexityText = arr.join(',')
        that.userSecurity.maxLength = response.data.maxLength
        that.userSecurity.maxLengthText = '最大长度' + response.data.maxLength
        that.userSecurity.minLength = response.data.minLength
        that.userSecurity.minLengthText = '最小长度' + response.data.minLength
      }).catch(() => {})
    }
  }
}
