...
import { approveStatusOptions, statusOptions, schemaStatusOptions } from './constants'
...
export default {
      ...
      listConfig: {
        ...
        rowHandle: {
          actions: [{
            ...
            hidden: (data) => {
              if (data.status === 'DISABLED') return true
              if (data.schemaStatus === 'WAIT' || data.schemaStatus === 'FAILED') {
                return false
              }
              return true
            }
  ...
  mounted() {
    this.loadData()
    console.info(this.$store.getters.tenant)
  },
  methods: {
    // 加载数据
    loadData() {
