<template>
  <div v-if="!tableHidden" class="dynamic-form-table">
          ...
          >
            ...
            <template v-for="(column,j) in displayColumns">
              <el-table-column
                ...
                :key="j"
                :prop="column.name"
                :label="$utils.isNotEmpty(column.field_options.units) ? column.label + '(' + column.field_options.units + ')' : column.label"
    ...
    <import-table
      ...
      @action-event="handleImportTablActionEvent"
    />
    <component
      :is="dialogTemplate"
      v-if="dialogTemplate"
...
export default {
  ...
  computed: {
    ...
    manageButtonWidth() {
      return this.manageButtons.length > 2 || this.manageButtons.length === 1 ? 60 : 140
    },
    mode() {
  ...
  methods: {
    ...
    handleImportTablActionEvent(file) {
